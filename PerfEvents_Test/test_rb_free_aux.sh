...
# 获取函数的地址
func_address=$(cat /proc/kallsyms | grep " rb_free_aux$" | awk '/rb_free_aux/ {print $1}')
test_ko=ko_rb_free_aux.ko

# 检查地址是否非零
chech_addr() {
    if [ "$func_address" != "0000000000000000" ] && [ -n "$func_address" ]; then
        log_info "rb_free_aux address: $func_address"
    else
        log_error "rb_free_aux address not found or is zero"
        exit 1
    fi
}
