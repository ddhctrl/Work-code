...
# 获取 notify_die 函数的地址
func_address=$(cat /proc/kallsyms | grep " do_no_restart_syscall$" | awk '{print $1}')
test_ko=ko_signal_no_restart_syscall.ko

# 检查地址是否非零
chech_addr() {
    if [ "$func_address" != "0000000000000000" ] && [ -n "$func_address" ]; then
        log_info "do_no_restart_syscall address: $func_address"
    else
        log_error "do_no_restart_syscall address not found or is zero"
        exit 1
    fi
}
