...
int main() {
    ...
    pe.sample_type = PERF_SAMPLE_CODE_PAGE_SIZE; // 设置采样类型为 PERF_SAMPLE_DATA_PAGE_SIZE
    pe.disabled = 1;
    pe.exclude_kernel = 1;
    pe.exclude_hv = 1;
    ...
    while (data_head != header->data_tail) {
        ...
        if (event->type == PERF_RECORD_SAMPLE) {
            ...
            uint64_t page_size_sampled = sample_data[0];
            printf("Sampled data page size: %lu\n", (unsigned long)page_size_sampled);
        }
