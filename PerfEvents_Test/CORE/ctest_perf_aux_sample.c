...
int main() {
    ...
    size_t aux_size = page_size * 4; // 4 pages for auxiliary data
    ...
    pe.sample_type = PERF_SAMPLE_AUX;
    pe.disabled = 1;
    pe.exclude_kernel = 1;
    pe.exclude_hv = 1;
    ...
    // 设置辅助数据缓冲区
    aux_buf = mmap(NULL, aux_size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    if (aux_buf == MAP_FAILED) {
        perror("aux mmap");
    ...
    while (data_head != header->data_tail) {
        ...
        if (event->type == PERF_RECORD_SAMPLE) {
            uint64_t aux_head = header->aux_head;
            uint64_t aux_tail = header->aux_tail;
            char *aux_data = (char *)aux_buf;

            printf("AUX data (first 16 bytes): ");
            for (size_t i = 0; i < 16 && aux_tail + i < aux_head; i++) {
                printf("%02x ", (unsigned char)aux_data[aux_tail + i]);
            }
            printf("\n");

            // 更新 aux_tail
            header->aux_tail = aux_head;
        }
        header->data_tail += event->size;
    }

    // 释放缓冲区
    munmap(aux_buf, aux_size);
    munmap(buf, mmap_size);
    close(fd);
    close(cgroup_fd);
