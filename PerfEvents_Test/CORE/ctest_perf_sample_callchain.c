...
int main() {
    ...
    pe.sample_type = PERF_SAMPLE_CALLCHAIN; // 设置采样类型为 PERF_SAMPLE_CALLCHAIN
    pe.disabled = 1;
    pe.exclude_kernel = 1;
    pe.exclude_hv = 1;
    ...
    while (data_head != header->data_tail) {
        ...
        if (event->type == PERF_RECORD_SAMPLE) {
            // 读取和解析调用链
            uint64_t *sample_data = (uint64_t *)((char *)event + sizeof(struct perf_event_header));
            uint64_t callchain_size = sample_data[0]; // 调用链的大小
            printf("Sampled callchain size: %lu\n", (unsigned long)callchain_size);
            for (uint64_t i = 1; i <= callchain_size; i++) {
                printf("Callchain %lu: %lx\n", i, sample_data[i]);
            }
        }
