...
# 获取 notify_die 函数的地址
func_address=$(cat /proc/kallsyms | grep " perf_event_read_event$" | awk '{print $1}')
test_ko=ko_perf_event_read_event.ko

# 检查地址是否非零
check_addr() {
    if [ "$func_address" != "0000000000000000" ] && [ -n "$func_address" ]; then
        log_info "perf_event_read_event address: $func_address"
    else
        log_error "perf_event_read_event address not found or is zero"
        exit 1
    fi
}
