...
# 获取 notify_die 函数的地址
func_address=$(cat /proc/kallsyms | grep " exclusive_event_installable$" | awk '{print $1}')
test_ko=ko_perf_event_match.ko

# 检查地址是否非零
check_addr() {
    if [ "$func_address" != "0000000000000000" ] && [ -n "$func_address" ]; then
        log_info "exclusive_event_installable address: $func_address"
    else
        log_error "exclusive_event_installable address not found or is zero"
        exit 1
    fi
}
