...
# 获取 notify_die 函数的地址
func_address=$(cat /proc/kallsyms | grep " perf_event_stop$" | awk '{print $1}')
test_ko=ko_perf_event_stop.ko

# 检查地址是否非零
check_addr() {
    if [ "$func_address" != "0000000000000000" ] && [ -n "$func_address" ]; then
        log_info "perf_event_stop address: $func_address"
    else
        log_error "perf_event_stop address not found or is zero"
        exit 1
    fi
}
