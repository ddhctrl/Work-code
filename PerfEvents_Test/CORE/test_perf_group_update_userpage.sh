...
# 获取 notify_die 函数的地址
func_address=$(cat /proc/kallsyms | grep " merge_sched_in$" | awk '{print $1}')
test_ko=ko_perf_group_update_userpage.ko

# 检查地址是否非零
check_addr() {
    if [ "$func_address" != "0000000000000000" ] && [ -n "$func_address" ]; then
        log_info "merge_sched_in address: $func_address"
    else
        log_error "merge_sched_in address not found or is zero"
        exit 1
    fi
}
