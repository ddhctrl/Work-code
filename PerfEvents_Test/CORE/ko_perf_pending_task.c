...
// 定义回调函数类型
typedef void(*perf_pending_task_t)(struct callback_head *head);

// 回调函数指针
static perf_pending_task_t perf_pending_task_callback = NULL;
...
static int __init test_module_init(void)
{
    ...
    perf_pending_task_callback = (perf_pending_task_t)addr1;
    perf_pending_task_callback(&event->pending_task);
    printk(KERN_INFO "call perf_pending_task success!\n");
