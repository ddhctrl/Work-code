...
int main() {
    ...
    uint64_t sample_regs_intr = (1ULL << PERF_REG_ARM64_X0) | (1ULL << PERF_REG_ARM64_X1); // 示例，指定需要采样的中断寄存器
    uint64_t val;
    ...
    pe.sample_regs_intr = sample_regs_intr;
    pe.disabled = 1;
    pe.exclude_kernel = 1;
    pe.exclude_hv = 1;
    ...
    while (data_head != header->data_tail) {
        ...
        if (event->type == PERF_RECORD_SAMPLE) {
            uint64_t *regs = (uint64_t *)((char *)event + sizeof(struct perf_event_header));
            uint64_t x0 = regs[PERF_REG_ARM64_X0];
            uint64_t x1 = regs[PERF_REG_ARM64_X1];
            printf("Sampled X0: %lx, X1: %lx\n", (unsigned long)x0, (unsigned long)x1);
        }
