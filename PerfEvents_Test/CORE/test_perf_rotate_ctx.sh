...
# 获取函数的地址
func_address=$(cat /proc/kallsyms | grep " rotate_ctx$" | awk '{print $1}')
test_ko=ko_perf_rotate_ctx.ko

# 检查地址是否非零
check_addr() {
    if [ "$func_address" != "0000000000000000" ] && [ -n "$func_address" ]; then
        log_info "rotate_ctx address: $func_address"
    else
        log_error "rotate_ctx address not found or is zero"
        exit 1
    fi
}
