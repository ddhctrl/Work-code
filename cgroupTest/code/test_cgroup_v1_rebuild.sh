...
#2 设置绑定cpu1
echo 1 > $CSPath/cpuset.cpus
print_message "INFO" "$CSPath/cpuset.cpus: "
cat $CSPath/cpuset.cpus
print_message "INFO" "$CSPath/cpuset.mems: "
cat $CSPath/cpuset.mems

#3 添加进程
./cgroup_fork_process &
PID=$!
CPID=$(pgrep -P $PID)
echo $CPID >> $CSPath/cgroup.procs
print_message "INFO" "$CSPath/cgroup.procs: "
cat $CSPath/cgroup.procs
