...
main() {
    ...
    echo "threaded" > $CPath/Example/cgroup.type
    log_info "$CPath/Example/cgroup.type: "
    cat $CPath/Example/cgroup.type

    ./cgroup_fork_process &
    PID=$!
    CPID=$(pgrep -P $PID)
    echo $CPID >>$CPath/Example/cgroup.threads

    log_info "$CPath/Example/cgroup.threads: "
    cat $CPath/Example/cgroup.threads
