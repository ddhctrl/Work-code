...
# 获取函数的地址
func_address=$(cat /proc/kallsyms | grep " cgroup_e_css$" | awk '{print $1}')
test_ko=cgroupv2_e_css.ko

# 检查地址是否非零
chech_addr() {
    if [ "$func_address" != "0000000000000000" ] && [ -n "$func_address" ]; then
        log_info "cgroup_e_css: $func_address"
    else
        log_error "cgroup_e_css not found or is zero"
        exit 1
    fi
}
